...
public abstract class ChartElement
{
    ...
    public virtual void CalculateTime([NotNull] List<Gimmick> bgmDataGimmicks)
    {
        ...
        {
            Debug.LogError(
                $"Cannot calculate Time of {this} because no bgmData has been set! Use CreateBgmData() to generate bgmData.");
            return;
        }

        int timeStamp = Measure * 1920 + Tick;
        Gimmick lastBgmData = bgmDataGimmicks.LastOrDefault(x => x.Measure * 1920 + x.Tick < timeStamp) ??
                              bgmDataGimmicks[0];
