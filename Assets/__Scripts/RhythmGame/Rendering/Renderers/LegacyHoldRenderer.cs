...
public class LegacyHoldRenderer : MonoBehaviour
{
    ...
    void Generate(HoldNote holdNote)
    {
        ...
        vertices = new Vector3[(holdNote.MaxSize + 1) * totalLength];
        uv = new Vector2[(holdNote.MaxSize + 1) * totalLength];
        ...
        for (int x = 0; x <= holdNote.MaxSize; x++)
        {
            for (int y = 0; y < totalLength; y++)
            {
                ...
                uv[vertexID] = GetUV(x, holdNote.MaxSize, y, totalLength);
                vertexID++;
            }
        }

        triangles = new int[holdNote.MaxSize * (totalLength - 1) * 6];
        ...
        for (int y = 0; y < holdNote.MaxSize; y++)
        {
            for (int x = 0; x < (totalLength - 1); x++)
            {
                ...
                triangles[tris + 1] = vert + (totalLength - 1) + 1;
                triangles[tris + 0] = vert + 1;
                
                triangles[tris + 5] = vert + 1;
                triangles[tris + 4] = vert + (totalLength - 1) + 1;
                triangles[tris + 3] = vert + (totalLength - 1) + 2;
